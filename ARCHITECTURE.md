...
**Open, priority order**: **#32** (P1, critical — standing umbrella tracking issue for the RPE-accuracy investigation; kept open by explicit user decision even though every phase and every child issue is currently closed); **#38** (accuracy/bug — NPOE ground-truth cross-validation found the known density-thin bias compounds severely over long single-BN re-entry propagations specifically; checkpoint only, not yet fixed); and the P4 backlog (#24 dashboard-follow-ups now tracked in `OREM-Watchlist`). #28 closed 2026-08-01 (gfortran now builds and passes 100%, CI matrix added); #24 closed 2026-08-01 (relocated to `OREM-Watchlist`#12, its real dependencies live there); #17 closed 2026-08-01 (performance benchmark: ~2-2.5 min/object end to end, re-entry propagation is the real bottleneck at 69-83% of runtime, not RSM as originally guessed — see README.md Version History). **385 tests across 12 suites**.
