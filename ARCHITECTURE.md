...
*(current as of v1.46, 2026-07-27; the Version History in README.md is the authoritative changelog)*
...
### Achieved Accuracy (current, v1.46) — and an honest note on where the numbers stand

Across the curated-7 validation set, **static atmosphere**: mean\|latest-zone RPE\| **25.9%**, mean\|ensemble RPE\| **1.9%** (median 1.2%) — see the real per-object table in §6. **With epoch-resolved weather + the diurnal bulge** (§4.1): mean\|latest-zone RPE\| **27.4%**, mean\|ensemble RPE\| **1.9%** (median 0.4%). Across the **97-object generalization set** (extended from 50, 2026-08-01 — §7): mean\|ensemble RPE\| **21.9%**, predict rate **61/97 (63%)**. The original 50 reproduce exactly (16.8%, 36/50=72%, unchanged by the extension); the 47 new objects — sourced from a genuinely harder, more historically diverse pool once the easy 2012+-decay candidates were exhausted (§7) — score worse on both metrics (29.2% mean, 25/47=53% predict rate), the expected generalization cost of not curating away harder real-world cases.

These are materially different from — and in the latest-zone metric's case, considerably worse than — the "2.4% median / 4.1% mean" figure this document quoted through v1.26. That older number came from an 8-zone-gated variant of the campaign that hasn't been the shipped default in a long time; the honest current picture is in §6, not a historical best case. **A genuinely useful, current finding**: under today's median-based ensemble metric (§5.5, changed from a mean in v1.44), the ensemble/spread indicator is now *consistently, and now even more* accurate than the "primary" latest-zone estimator on this curated-7 set — the opposite of the v1.20 finding that made latest-zone primary in the first place, and the gap widened further after v1.46's revert below. See §5.5 for why, and treat "primary" as the officially-reported estimate, not necessarily the most accurate one for every object.

The accuracy story since v1.26 has been one of closing generalization gaps rather than one clean number going up: a `zone_select` recency fix (v1.41), a literature-grounded diurnal density bulge (v1.42, §4.1), a widened `max_zone_days` default (10→20 days, v1.43, raising the 50-object predict rate from 58%→72% before the Falcon-9 swap below changed the roster again), a BSTAR-informed per-zone BN prior (**G3**, §5.1/5.3), and the mean→median ensemble-metric switch (v1.44) all measurably helped *something* while each also interacted unpredictably with the BN-range carryover mechanism across zones (§5.3) — four independent, confirmed instances of the same fragility (issue #35). v1.45 removed three Falcon-9 R/B objects from the 50-object set (non-maneuverable substitutes, since `propagate_ks` has no thrust/maneuver modeling and SpaceX is documented to perform active post-separation deorbit burns on Falcon 9 second stages). **Most recently (v1.46)**: the 4th fragility instance — issue #31's noise-matched-apobs fix, re-tested under today's full stack (issue #34) and found to regress both metrics rather than the small net-positive result that originally justified keeping it — was reverted outright (§4 step 5a, §5.3); separately, issue #35's own investigated fix (a non-recursive pooled-BN-prior replacement for the carryover mechanism) was reviewed and explicitly discarded, not shipped, leaving the recursive carryover itself still structurally unresolved.
...
The GA **never calls the propagator** — it evaluates bilinear interpolation of the pre-computed surfaces. This is what makes OREM computationally feasible, and it's also the source of the interpolation-optimism gap characterized in issue #36 (Finding 3, generalized) and confirmed as inherited from the method's own founding literature in issue #39 — no document in that literature's 2006-2019 lineage validates a GA-converged optimum against a fresh direct propagation, and the one sub-lineage that did re-propagate every candidate directly (no RSM step) is the only one with real, independently-checked accuracy figures.
...
**Honest current caveat**: on today's curated-7 static campaign, the median-based ensemble metric is now *more* accurate than the "primary" latest-zone estimator for all 7 objects (§1, §6) — the opposite of the relationship that made latest-zone primary in v1.20. This hasn't been re-evaluated as a possible change to which estimate is reported as PRIMARY; it's flagged here as a real, current data point rather than acted on.

---

## 6. Validation Cases & Results (current, v1.46, `scratch_rpe/rpe_campaign_7obj_issue31.csv`, static atmosphere, 8-zone)

| Object | NORAD | i (°) | e₀ | Known re-entry | Latest-zone RPE | Ensemble (median) RPE |
|---|---|---|---|---|---|---|
| PSLV-C39 R/B | 42928 | 19.2 | 0.33 | 2019-03-03 | −22.0% | **1.3%** |
| Ariane 5 ESC-A | 35497 | 5.7 | 0.63 | 2016-10-31 | −76.4% | **2.8%** |
| Proton-M Briz-M | 39615 | 48.5 | 0.68 | 2017-09-15 | 19.8% | **−0.1%** |
| GSLV R/B | 32007 | 25.9 | 0.29 | 2010-06-06 | 2.0% | **−0.2%** |
| Proton-M R/B | 37819 | 63.4 | 0.47 | 2013-09-12 | 31.9% | **−7.7%** |
| Long March 3B | 37151 | 24.9 | 0.56 | 2015-12-03 | −24.2% | **−1.2%** |
| Ariane 5 R/B | 27526 | 17.7 | 0.59 | 2012-05-09 | 4.7% | **0.3%** |
| **mean\|·\|** | | | | | **25.9%** | **1.9%** |
| **median\|·\|** | | | | | **22.0%** | **1.2%** |

The ensemble (median) column beats the "primary" latest-zone column for all 7 objects here (§5.5) — a genuine, current reversal from the relationship that made latest-zone primary, worth knowing when reading any single object's report rather than trusting the label alone. With epoch-resolved weather + the diurnal bulge (`scratch_rpe/rpe_campaign_weather.csv`): mean\|latest\|=27.4%, mean\|ensemble\|=1.9%, median\|ensemble\|=0.4% — a similar pattern, non-uniform per object (§1).

35497 is a persistent outlier on both metrics (i=5.7° solar-apsidal-resonance case) and is also the one object whose ensemble metric regressed under every pool-size variant tested in the issue #35 carryover-chain investigation (§5.3, closed 2026-07-30) — worth investigating on its own terms if accuracy work resumes, since pool size alone doesn't move it. A dedicated 3-variable (e, A, i) optimization was proposed for this case (issue #9) but closed 2026-07-30 without being pursued — not attempted, not rejected on evidence, just deprioritized.

---

## 7. Development Status

Core algorithm **complete** (all closed): #1–#8 pipeline, #12 diagnostics/identifiability, #13 report, #16 E2E + accuracy target, #25 drag audit, #26 epoch-resolved space weather (§4.1), #27 critical-inclination decay (root-caused as a BN-identifiability issue, not a gravity-model gap — see git/issue history for the GMAT cross-check that resolved it), KSROP #24, #29 (nzones_max generalization gap — decay-phase-proximity explains it, raising nzones_max tested and rejected), #34/#35/#36 (BN-carryover/GA-RSM investigations, closed 2026-07-30), #9 (3-variable optimization, closed without implementation), #14 (density model validation, closed 2026-07-30 — all 4 scope items done/dispositioned, diurnal bulge shipped, ISO/CD 27852 claim corrected), #37 (fully-osculating fit-target, tested and rejected 2026-07-30 — regresses curated-7 cleanly, completes both directions of the hybrid-basis question first raised by #31), #15 (new-object operational workflow, closed 2026-07-30 — `OPERATIONS.md` shipped, grounded in OREM-Watchlist's real running pipeline).

**Open, priority order**: **#32** (P1, critical — standing umbrella tracking issue for the RPE-accuracy investigation; kept open by explicit user decision even though every phase and every child issue is currently closed); and the P4 backlog (#24 dashboard-follow-ups now tracked in `OREM-Watchlist`). #28 closed 2026-08-01 (gfortran now builds and passes 100%, CI matrix added); #24 closed 2026-08-01 (relocated to `OREM-Watchlist`#7, its real dependencies live there); #17 closed 2026-08-01 (performance benchmark: ~2-2.5 min/object end to end, re-entry propagation is the real bottleneck at 69-83% of runtime, not RSM as originally guessed — see README.md Version History); #38 closed 2026-08-01 (NPOE ground-truth cross-validation confirmed the known density-thin bias independently, and quantified that it doesn't materially affect the reported primary/ensemble estimates — their horizons are too short for it to compound — see README.md Version History); #39 closed 2026-08-01 (literature survey confirms the RSM/GA interpolation-optimism gap, issue #36 Finding 3, is inherited from the method's own founding literature — the closest published analog, on a real object, made predictions independently confirmed 4-7% off actual, uncaught by its own authors — not an OREM-specific defect; see README.md Version History). **385 tests across 12 suites**.

**RPE campaign extended 50 → 97 objects (2026-08-01)**: the established sourcing filter (2012+ decay, ROCKET BODY/DEBRIS, apogee>8000km/perigee<3000km against the current satcat.csv) returned essentially zero new candidates beyond more of the already-heavily-used SL-12 debris cluster — the easily-findable pool under those exact criteria is exhausted. Relaxed in two explicit-decision steps: dropped the decay-date floor (any 1961-2025 decay), then raised the apogee ceiling to the already-established 280,000km TLI-exclusion bound and included PAYLOAD (old non-maneuverable dead satellites) for the first time. Every candidate was screened with the *real* `zone_select` algorithm (not a raw TLE-count heuristic) plus the same last-TLE-to-decay-gap and cluster-independence bars every prior round used (new families capped at 2 members). Final: 47 new objects (21 diverse historical, 19 diverse+PAYLOAD, 7 SL-12 top-up) — 3 short of round-number 100, not padded further. Full sourcing methodology in `scratch_rpe/rpe_campaign.F`'s own header comment. See §1 for results.
