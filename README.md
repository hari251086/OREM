...
**2026-09-07 — Issue #56: solar apsidal "U-turn" resonance (Wang & Gurfil 2017) sweep across the full 97-object campaign, plus a restricted campaign that predicts using ONLY post-resonance TLE data.** `scratch_rpe/resonance_campaign_sweep.py` generalizes the earlier single-object #27/#32 Phase-3 check (33587) to every object in `scratch_rpe/rpe_campaign.F`: computes `a(t), e(t), i(t)` straight from real TLE mean elements (no propagation) and flags every crossing of the paper's Eq. 12 resonance condition (`a(1-e^2)^(4/7)` vs. a computable critical value from J2/inclination/solar mean motion) -- the U-turn is exactly this crossing, where the ~180-day periodic eccentricity oscillation turns into a monotonic one, per the paper the near-universal prelude to final re-entry. **16 of 97 objects show a genuine crossing** (`scratch_rpe/resonance_sweep_97obj.csv`): 35497, 37151, 27526, 37819, 59347, 40943, 60328, 18983, 13112, 28141, 19881, 5980, 4570, 10724, 28140, 44911 (11 in the paper's validated low-inclination regime i<46.4°, 5 near-critical-inclination i~59-66° debris flagged as extrapolation but kept per explicit user decision).

New driver `scratch_rpe/rpe_campaign_resonance16.F` (identical `orem_run` parameters to the main campaign) runs only these 16, with `scratch_rpe/make_postresonance_tles.py` truncating each object's TLE input (`input/example_<norad>_postresonance.tle.txt`) to only records at/after that object's own detected resonance epoch -- `zone_select`/the GA never see pre-resonance data for these objects, an explicit hard operational rule. 4 of the 16 have zero post-resonance TLEs at all (resonance coincides with the object's last tracked TLE) and 3 more have too few (1-7, below `zone_select`'s `min_pts=8`) -- both reported as `ierr` failures rather than silently backfilled with disallowed pre-resonance data. Of the remaining 9, 8 produce a real post-resonance-only prediction (`scratch_rpe/rpe_campaign_resonance16.csv`).

**Result, genuinely mixed, and a direct quantitative confirmation of the paper's own claim on OREM's real pipeline**: vs. the unrestricted full-history baseline (`scratch_rpe/rpe_campaign_new90.csv`), restricting to post-resonance-only data measurably *degrades* ensemble RPE for 35497 (2.8%→24.9%), 37151 (-1.2%→36.5%), and 40943 (11.5%→40.8%) -- exactly the paper's predicted effect, that dynamics are far more sensitive/harder to fit after the resonance than before it. 27526/13112 are roughly flat, 37819/59347 slightly improve, and 10724 gets a valid prediction (22.2%) where the full-history baseline produced none at all (`reentry_jd=0`). Not shipped as a pipeline change -- an investigative campaign extension, same category as Phase 3/#27, with the full per-object table and methodology in issue #56.

**2026-09-07 (same day) — Issue #56 follow-up: candidate pool expanded beyond 97, 180 new objects screened/fetched, 11 more resonance hits found, all fail to produce a post-resonance-only prediction.** `scratch_rpe/screen_new_candidates.py` re-applied the campaign's own established SATCAT filter (APOGEE>8000km, PERIGEE<3000km, APOGEE<=280,000km, ROCKET BODY/DEBRIS/PAYLOAD, real DECAY date) against a local cached SATCAT snapshot, excluding the 97 already used -- **180 new candidates** passed (58 already had TLE history cached locally via OREM-Watchlist's `objects/` store, zero new Space-Track requests; 122 needed a first-ever fetch). `scratch_rpe/fetch_new_candidates.py` pulled those 122 in just 4 batched `gp_history` calls (the `spacetrack` library joins a Python list of NORAD IDs into one comma-delimited query -- SPACE_TRACK_USAGE_POLICY.md §4's query-batching requirement, not the older per-object-loop precedent), via the same shared rate-limited `heowatch.spacetrack_client.get_client()` every other script in this lineage uses -- 8 objects returned no `gp_history` data at all (too old / never tracked at that granularity), the other 172 got a real TLE file.

`scratch_rpe/resonance_sweep_new_candidates.py` (same Eq. 12 detection as the 97-object sweep) found **11 more resonance crossings** among the 180 (`scratch_rpe/resonance_sweep_newcand.csv`). 4 excluded before running: 3 are Falcon 9 R/Bs (39501/44187/42985) -- this project's established no-active-maneuvering policy (issue #29 follow-up, since OREM's physics is drag-only); 1 (13913) has an internally inconsistent SATCAT decay date (last tracked TLE ~3.4yr *after* its catalogued decay, same exclusion class as 9892 in the original 97-object round). The remaining 7 (7096, 68538, 6294, 18954, 29462, 5941, 6231) were run through `scratch_rpe/rpe_campaign_resonance_new7.F` against their own post-resonance-only TLE truncations, same hard rule and `orem_run` parameters as `rpe_campaign_resonance16.F`.

**Result: all 7 fail with `ierr=2` (no valid zone found)** -- even 6231 (405 post-resonance TLEs spanning ~4.9 years) can't form a single `zone_select` window meeting the production `min_pts=8`/`max_zone_days=20`/`R2>=0.90` bar; the other 6 have too little post-resonance data at all (1-12 TLEs). A real, disclosed negative result, not a bug: this pool is disproportionately 1970s-80s objects with much sparser historical tracking than the original curated/well-tracked set, and the hard rule's post-resonance-only restriction removes exactly the denser pre-resonance data these particular objects depended on to ever form a zone. **Combined total across both pools: 23 objects (of 277 screened) show a genuine U-turn resonance crossing; 8 (all from the original 97) produce a valid post-resonance-only OREM prediction.** Full per-object detail in issue #56's follow-up comment.

**2026-09-08 — Issue #56: Wang & Gurfil-style plot PDF for all 23 resonant objects, plotted in Scilab.** `scratch_rpe/resonance_plots/prepare_plot_data.py` computes, per object, a real TLE-derived time series (`a(t), e(t), i(t)`, RAAN/argument-of-perigee straight from each TLE's mean elements, no propagation) plus a resonance angle `psi(t) = mod(RAAN(t)+argp(t)-L_sun(t), 360deg)` -- a TLE-computable proxy for the paper's solar-azimuth-vs-apsidal-line angle, since the paper's own resonance condition (Eq. 9) is exactly `d(RAAN+argp)/dt ~= dM_solar/dt`, so the same U-turn signature (angle rises, reverses, falls) shows up in this angle too. `scratch_rpe/resonance_plots/plot_resonance.sce` (Scilab, batch-mode via `WScilex-cli.exe -nb -f ... -quit` -- `-nw`/`-nwni` are rejected by this build's CLI executable, which is already headless) renders a 6-panel page per object: the resonance-angle U-turn, perigee height with the resonance epoch and 100km decay threshold marked, the `(a,e)`-plane trajectory with the resonance-condition curve (Eq. 12) and decay-condition curve overlaid, `a(t)`, `e(t)`, and an info panel -- directly analogous to the paper's own Figs. 4-7/8-11/12-15 example sets, but built from 23 independent real objects instead of one simulated GTO swept over RAAN. `plot_frontmatter.sce` adds a title page and an index table. `merge_pdf.py` (`pypdf`, install-only use -- all plotting is Scilab) concatenates all 25 pages into `scratch_rpe/resonance_plots/OREM_solar_apsidal_resonance_plots.pdf`. Per-object PDF pages (`pages/`, pure merge byproducts) are gitignored; the per-object data CSVs, manifest, scripts, and the final merged PDF are tracked.

**2026-09-08 (same day) — Issue #56: same plot exercise extended to the FULL 253-object GTO/HEO pool, for manual visual review instead of trusting the automated detector alone.** User feedback: some of the 23 auto-flagged objects don't read as a genuine U-turn on inspection -- the Eq. 12 crossing test is a necessary but not sufficient signature (a lone crossing can come from TLE noise, a very short record, or a regime the paper's own low-inclination derivation doesn't cleanly cover). Rather than tune the detector further, generated the identical 6-panel page for **every** object in the screened pool (277 total; 253 have >=3 TLEs and get a page, 24 are skipped for having too few points to draw a trajectory at all) via `scratch_rpe/resonance_plots/prepare_plot_data_fullpool.py` + `plot_resonance_fullpool.sce` + `plot_frontmatter_fullpool.sce` (11-page paginated index, since 253 rows don't fit one page) + `merge_pdf_fullpool.py`. Each page still shows the automated DETECTOR verdict (resonant or not, from the same Eq. 12 check) as a hint in the info panel, and still draws the resonance-condition/decay-condition curves regardless of verdict, so a missed or a spurious case is visually checkable either way.

This version deliberately **re-includes the 4 objects the restricted-campaign work had excluded by policy** (3 Falcon 9 R/Bs, 1 object with an internally inconsistent SATCAT decay date) -- flagged with a `NOTE:` in the info panel instead of being silently dropped, since the point of this pass is unfiltered manual judgment, not an automated pipeline decision. Long TLE records (several exceed 5,000-11,000 points) are uniformly decimated to <=800 plotted points per object (`prepare_plot_data_fullpool.py`'s `decimate()`) to keep per-page file size and Scilab render time bounded -- the U-turn/oscillation pattern this is for is a low-frequency signal, so decimation doesn't lose the signature. The merged 265-page PDF is 48.7MB, over the file-send size limit, so `split_pdf_fullpool.py` splits it into two ~24-25MB parts by page count. **Not committed to the repo** (`.gitignore`: `pages_fullpool/` and `OREM_solar_apsidal_resonance_plots_FULLPOOL*.pdf`) -- a one-time manual-review deliverable sent directly to the user, not a permanent artifact; the per-object data CSVs (`data_fullpool/`), manifest, and all four scripts are tracked so it's fully reproducible.

**2026-09-08 (same day) — Issue #56: TLE fetch script audited for a truncation bug after the user flagged several 1-2-TLE objects as suspiciously low; none found.** `scratch_rpe/fetch_new_candidates.py` batches up to 40 NORAD IDs into one `gp_history` call -- a plausible bug shape is the batched query silently truncating results for objects late in the sort order if the combined response hit a server-side size/time limit. `scratch_rpe/diag_low_tle_refetch.py` re-fetched the 12 objects that came back with only 1-2 TLEs from the batched fetch, this time one object per request with a 5-second pause between requests (a deliberately generous wait, ruling out any rate-limit/timeout-driven partial response), and compared counts. **All 12 matched exactly** (`scratch_rpe/diag_low_tle_refetch_results.csv`) -- e.g. 17 (THOR ABLE R/B, 1958-era), 6068 (PROGNOZ 2), 11700 (COSMOS 1164), several SL-12 DEB fragments (28014/28015/28037), all returned the identical count solo as they did batched. Also spot-checked 3 large objects (4336: 729, 10370: 1776, 6877: 887) to confirm the batching doesn't silently truncate bigger responses either -- exact matches on all three. **Verdict: no bug in the fetch script.** These objects' sparse historical TLE counts are genuine -- either very-early space-age objects (1958-60s, sparse contemporaneous tracking), old debris fragments that were only briefly cataloged, or (68539/68541) objects launched in 2026 that simply don't have more TLEs yet. No code change; this is a verification result, not a fix.

**2026-09-09 — Issue #56 capstone: critical review of whether the solar apsidal U-turn resonance is a good theory *and* a good measure for re-entry prediction, using everything gathered in this investigation.** Two different questions, two different answers.

The underlying mechanism (J2 apsidal-precession rate becoming commensurate with the Sun's mean motion, Eq. 12) is real and well-derived -- as an *explanation* for why some GTOs' eccentricity collapses from periodic to monotonic, it's legitimate; the qualitative signature does show up in real TLE-derived plots where a crossing is detected.

As a *general claim about GTO/HEO evolution*, it's weaker than the paper's own framing suggests: only **10.7% (27/253)** of the full screened pool shows a detectable crossing at all; restricted to the paper's own validated low-inclination regime (i<46.4°) that rises to **26.2% (17/65)** -- a real concentration relative to the high-inclination majority (**5.3%, 10/188**, mostly Molniya/Proton-debris, a population the paper never modeled) -- but still a minority phenomenon, not the near-universal "prelude" the paper's language implies.

The "prelude to re-entry" *timing* claim doesn't hold up either: resonance-to-decay gaps across the 27 detections range from essentially 0 days to 8.3 years (4 within 1 day of the terminal TLE itself -- more likely an artifact of `a`/`e` both collapsing under terminal drag than a distinct predictive event -- 11 within 1-30 days, 2 within 30-365 days, 7 within 365-1000 days, 3 beyond 1000 days).

The strongest evidence is the direct empirical test already run on OREM's own pipeline: the "hard rule" restricted campaign (`rpe_campaign_resonance16.F`, predict only from the resonance epoch onward) **degraded** accuracy on the 7 directly-comparable objects -- mean |ensemble RPE| went from 4.74% (full-history baseline) to 22.47% (post-resonance-only), roughly a 5x regression, 6 of 7 objects worse. This is *consistent* with the paper's own caveat that post-resonance dynamics are more sensitive/harder to predict, but inverts the practical conclusion: treating the resonance epoch as "start predicting from here" actively hurts accuracy rather than helping it.

**Verdict: a real and elegant theory in its validated low-inclination niche, but not a good measure for re-entry prediction** -- it doesn't generalize across the broader debris population, its timing relative to decay is too variable to serve as an operational early-warning signal, and operationalizing it as a prediction trigger measurably hurt accuracy rather than improving it. Its practical value is closer to a diagnostic flag that uncertainty is about to increase than a tool for making a better prediction. Full write-up: issue #56.

**2026-09-25 — Issue #56: full 253-object manual visual review completed; zone-selection literature review; RPE accuracy gap diagnosed and applied to production.** Three linked pieces of work:

1. **Manual review of the automated Eq. 12 detector's own accuracy** (`scratch_rpe/resonance_plots/OREM_solar_apsidal_resonance_plots_FULLPOOL.pdf`, all 253 pages read): of the 27 objects the detector flagged, only 7-8 show a genuine U-turn on visual inspection (40943, 59347, 35497, 10724 strong; 37151, 27526, 18954 weaker but plausible; 19881 uncertain) -- 15 are false positives, mostly the detector firing on terminal orbital collapse (last few TLEs before decay mimic the resonance signature) or single noisy TLEs, not real resonance. Real detection rate is ~3% (7-8/253), not the raw 10.7% (27/253) -- strengthens rather than weakens the capstone finding above, since it rests on confirmed-genuine objects. A curated 27-object-only PDF (`OREM_resonance_flagged27_review.pdf`, grouped by verdict) was built for any follow-up review. Full classification table and suggested detector hardening (terminal-TLE guard, `a(t)` outlier filter) posted to issue #56.
2. **Zone-selection methodology literature review** (`scratch_rpe/zone_selection_literature_review.md`): deep read of ISRO Monograph-2024 Chapter 8 (26 studies) with page-cited extraction of every study's zone/window definition, TLE counts, event dependence, and accuracy. Finding: no other study in this lineage documents a reproducible, parameterized zone-selection algorithm the way `zone_select.F` does -- every other "zone" (Mutyalarao & Sharma's 8-zone/200-day split, Sellamuthu & Sharma's 5 HEO zones, Fletcher & Sharma's 4 intervals) is described as visually/"roughly" segmented, a genuine methodological step beyond OREM's own literature ancestry, not a gap relative to it.
3. **RPE accuracy gap diagnosed** (`scratch_rpe/rpe_accuracy_review.md`): a 340-zone mechanical full-tiling validation campaign (7 confirmed-resonant objects, `scratch_rpe/validation_RPE/`) showed individual-zone RPE of 40-400%+ and only 44% of zones converging at all -- traced to the campaign driver running every zone as an *isolated* `orem_run` call (`nzones_max=1`), which structurally discards OREM's own validated multi-zone median ensemble and BN trust-gated carryover (`t_std=0.00` in every one of 340 rows proves zero ensemble averaging ever occurred). GA hyperparameters and eccentricity-bound narrowing were checked against the COSPAR ASR precursor work (`E:\Research\1. R&D\Re-entry\COSPAR ASR`, a pre-OREM Scilab+Fortran GENESIS-GA pipeline) and found to already be effectively identical -- not the cause. Applying the real fix (multi-zone `orem_run` calls, not isolated single-zone ones) to OREM-Watchlist's production `backfill_validation.py` for these 7 objects, computed via the same `compute_validation_rpe` the live dashboard uses, gives 27526 0.51%, 35497 1.88%, 37151 0.23%, 40943 1.62%, 59347 -12.27%, with 10724/18954 failing outright (105- and 27-TLE records too sparse for any valid zone) -- a data-availability limit, not a config problem. No `src/` code changes were needed; OREM's own production defaults were already correct, the bug was in the diagnostic campaign script's own design.

**2026-09-26 — Documented the physical basis for `orem.F`'s eccentricity search range** (`src/orem.F` comment, `ALGORITHM.md` §4c, this section's References). The `de` half-width computed before every RSM/GA fit (centered on the zone's own first-TLE eccentricity, sized to that zone's own TLE-to-TLE scatter) was previously undocumented as to *why* that's the right approach. Traced to two independently-arrived-at, now cross-corroborating sources: Sellamuthu's PhD thesis (Ch.5, pp.141-142) shows the pre-OREM manual method actually bounded APOGEE altitude (±3.5 km) and back-derived the eccentricity bound from it via apogee=a(1+e) -- e.g. Table 5.4's 35497 Case (ii) Zone 1 bound `[0.6292873,0.6296835]` is exactly a ±3.5 km apogee window, the same bound later hand-typed into the COSPAR-ASR `gene9.txt` GA configs found this session. Früh & Schildknecht (2012, *J. Guidance Control Dyn.* 35(5), 1483-1491), validating TLEs against independent optical tracking, supply the physical reason apogee (not perigee) is the well-constrained quantity to bound: "HEO objects are observed mainly in apogee, which may result[s] in orbits which are not well defined in perigee" (p.1484) -- ground-station tracking opportunity concentrates near apogee (slower angular motion there, Kepler's 2nd law). This independently corroborates the same thesis's own separate finding (p.139) that mean apogee evolves smoothly while mean perigee is erratic -- the reason this whole pipeline fits apogee, not perigee, throughout. Directly answers manuscript issue #47 (Reviewer 1's "provide a recipe" / Reviewer 3's "was window length decided based on min/max length?") and issue #53 (Reviewer's "be specific about what TLE shortcomings you mean, don't just cite Früh & Schildknecht as 'common wisdom'" -- this *is* the specific, mechanistic citation). No behavior change; comment/documentation only, 388/388 tests still pass.
...
- Sellamuthu, H. & Sharma, R.K. (2018) Optimal re-entry time prediction of resident space objects from highly elliptical orbits, 42nd COSPAR Scientific Assembly, Abstract PEDAS.1-18-18 — KSROP (the same KS-regularized propagator lineage this repo depends on) applied to HEO re-entry via 5 hand-selected zones + RSM/GA, max zone error 5.78%; OREM's most direct methodological ancestor found in the zone-selection literature review (`scratch_rpe/zone_selection_literature_review.md`)
- Sharma, R.K., Anil Kumar, A.K. & Adimurthy, V. (2024) Space Debris and Space Situational Awareness Research Studies in ISRO, Monograph-2024, Indian Space Research Organisation — Chapter 8 surveys 26 re-entry prediction studies; no other study documents a reproducible, parameterized zone-selection algorithm the way `zone_select.F` does, and its own multi-method benchmark (Dutt et al. 2020/2021/2023) found no single method dominates across the re-entry horizon, a literature-backed explanation for a single-method (RSM-GA-only) system's expected accuracy ceiling
- Früh, C. & Schildknecht, T. (2012) Accuracy of Two-Line-Element Data for Geostationary and High-Eccentricity Orbits, Journal of Guidance, Control, and Dynamics 35(5), 1483–1491, DOI: 10.2514/1.55843 — independent optical-tracking validation finding "HEO objects are observed mainly in apogee, which may result[s] in orbits which are not well defined in perigee" (p.1484); the documented physical reason OREM's eccentricity search range is derived from an apogee-altitude tolerance (`orem.F`, `ALGORITHM.md` §4c) and why this pipeline fits mean apogee, not perigee, as its primary decay signal throughout
